...
func main() {
	...
	// 添加工具
	{
		...
		s.AddTool(calculatorTool, func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			op := request.Params.Arguments.(map[string]any)["operation"].(string)
			x := request.Params.Arguments.(map[string]any)["x"].(float64)
			y := request.Params.Arguments.(map[string]any)["y"].(float64)
			var result float64
			switch op {
			case "add":
				result = x + y
	...
	sseServer := server.NewSSEServer(
		s,
		server.WithBaseURL(fmt.Sprintf("http://%s", "localhost:9000")),
	)
